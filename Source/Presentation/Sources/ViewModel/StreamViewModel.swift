...
public final class DefaultStreamViewModel: StreamViewModel {
    ...
    public func configure(roomRole: RoomRole, outputView: UIView?) async {
        await streamUseCase.configure(
            ...
            orientation: UIDevice.current.orientation.avCaptureVideoOrientation ?? .landscapeLeft,
    ...
    public func setScreenSize() async {
        if let orientation = await UIDevice.current.orientation.avCaptureVideoOrientation {
            await streamUseCase.setScreenSize(orientation: orientation)
        }
    }
    ...
    @MainActor @objc private func orientationDidChange(_ notification: Notification) {
        Task {
            if let orientation = UIDevice.current.orientation.avCaptureVideoOrientation {
                await streamUseCase.setVideoOrientation(orientation)
            }
        }
    }
