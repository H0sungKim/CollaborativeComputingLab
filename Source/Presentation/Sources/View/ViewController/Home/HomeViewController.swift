...
public final class HomeViewController: UIViewController {
    ...
    private func presentCreateRoomAlert() {
        ...
        let actionCreate: UIAlertAction = UIAlertAction(title: "생성", style: .default, handler: { [weak self] _ in
            ...
            guard let roomViewController = self?.diNavigationController?.viewControllerFactory.buildRoomViewController(id: id, userName: userName, role: .instructor, roomViewModel: self?.roomViewModel, chatViewModel: nil, streamViewModel: nil) else { return }
...
extension HomeViewController: UITableViewDelegate {
    ...
    private func presentEnterRoomAlert(roomEntity: RoomEntity) {
        ...
        let actionEnter: UIAlertAction = UIAlertAction(title: "입장", style: .default, handler: { [weak self] _ in
            ...
            guard let roomViewController = self?.diNavigationController?.viewControllerFactory.buildRoomViewController(id: roomEntity.id, userName: userName, role: .student, roomViewModel: self?.roomViewModel, chatViewModel: nil, streamViewModel: nil) else { return }
            Log.i(self?.roomViewModel.participants.value)
            self?.navigationController?.pushViewController(roomViewController, animated: true)
            self?.dismiss(animated: true, completion: nil)
        })
