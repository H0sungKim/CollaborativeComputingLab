...
\begin{document}
%
\title{Live Mobile Learning System with Resizable Split Panes}
%
...
\keywords{Live mobile learning \and Whiteboard \and Annotation \and Resizable split panes}
\end{abstract}
%
%
%
...
The system consists of a server and tablet clients. The server is composed of signaling server and a streaming server.
The signaling server is responsible for centralized session management and real-time coordination among clients. It is implemented using the NWProtocolWebSocket of Swift Network Framework\cite{Network} and runs as an Xcode Command Line Tool. The signaling server provides multi-classroom functionalities, including classroom creation, termination, joining, leaving and chat. It handles multiple message types by using Codable based enum and manages multiple classrooms using a dictionary keyed by UUIDs.
The streaming server operates as a separate process from the signaling server but is integrated into a single Xcode target. It is executed by invoking the NGINX executable along with its configuration file through Swift's Process() API. It uses the RTMP (Real-Time Messaging Protocol)\cite{RTMP} to receive video streams including presentation slides with annotation, whiteboard, instructor camera video, and audio from the instructor client, and delivers them to student clients in real time.

Figure~\ref{fig2} shows the home screen. Users can either join an existing classroom as a student or create a new room using the "+" button to start a lecture as an instructor. The room list is updated in real time via communication with the signaling server, and users can also manually refresh the list by pulling down the UITableView. Both room creation and entry are handled by sending an Enter Room message to the server.

\begin{figure}
\includegraphics[width=\textwidth]{fig2.png}
\caption{User interface of a home view} \label{fig2}
\end{figure}

\noindent
Figure~\ref{fig3} shows the instructor client's classroom interface. The UI consists of presentation slides, a whiteboard, instructor camera video, a participant list, and a chat. Detailed descriptions of each component are as follows.

\begin{figure}
\includegraphics[width=\textwidth]{fig3.jpg}
\caption{User interface of a classroom view} \label{fig3}
\end{figure}
...
Annotation on slides is implemented using PDFPageOverlayViewProvider, where a PKCanvasView of PencilKit\cite{PencilKit} is added to the \verb+overlayView+ of each \verb+PDFPage+. Pen and eraser tools are implemented as toggle buttons, which switch the \verb+PKTool+ accordingly. When both tools are disabled, the view behaves as a scrollable PDF document.
...
\caption{User interface of a classroom view with participants panel hidden} \label{fig4}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{fig5.png}
\caption{User interface of a classroom view with participants and chat panels hidden} \label{fig5}
\end{figure}
...
\caption{User interface of a classroom view an expanded presentation slide area} \label{fig6}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{fig7.png}
\caption{User interface of a classroom view with an expanded whiteboard area} \label{fig7}
\end{figure}
...
\toprule%
Target System & \multicolumn{5}{@{}c@{}}{Respondents} & Mean & SD \\\cmidrule{2-6}
& A & B & C & D & E & & \\
\midrule
Webex  & 5 & 5 & 5 & 5 & 5 & 5 & 0 \\
5:5 Fixed UI  & 1 & 5 & 1 & 2 & 4 & 2.6 & 1.82 \\
User-Adjustable UI  & 5 & 5 & 5 & 5 & 5 & 5 & 0 \\
\bottomrule
\end{tabular*}
\end{table}
...
& A & B & C & D & E & & \\
\midrule
Webex  & 3 & 5 & 5 & 4 & 4 & 4.2 & 0.84 \\
5:5 Fixed UI  & 1 & 3 & 2 & 3 & 5 & 2.8 & 1.49 \\
User-Adjustable UI  & 5 & 4 & 4 & 5 & 5 & 4.6 & 0.55 \\
\bottomrule
\end{tabular*}
\end{table}
...
\midrule
Webex  & 5 & 5 & 3 & 4 & 3 & 4 & 1 \\
5:5 Fixed UI  & 3 & 2 & 2 & 1 & 4 & 2.4 & 1.15 \\
User-Adjustable UI  & 4 & 4 & 5 & 5 & 5 & 4.6 & 0.55 \\
\bottomrule
\end{tabular*}
\end{table}
